...
import keras
import sys
from keras.layers import Input, Conv3D, Activation, AveragePooling3D, Flatten, Dense, Lambda
...
def build_mini():
    ...
    x = AveragePooling3D(pool_size=(2, 2))(x)
...
def build_test():
    ...
    x = img_input                                      
    x = Conv3D(3, (3, 3), padding='same')(x)
    x = AveragePooling2D((2, 2), 2, padding='valid')(x)
    x = Activation('relu')(x)
    x = Conv3D(16, (1, 1), padding='valid')(x)
...
for i, layer in enumerate(model.layers):
    if "conv3d" in layer.name:
        ...
        for prev_i in range(i, 0, -1):
            ...
            if ("Conv3D" in layer_name) or ("average_pooling3d" in layer_name) or prev_i == 0:
